  ...
  async function saveBlueprintDraft() {
    ...
    try {
      ...
      setState(current => current
        ...
        : current)
      setProgress('success')
      setDraftMessage('已保存到 intake-analysis.md，P8 / P9 / P15 会读取更新后的蓝图。')
    } catch (err: any) {
      ...
      <div className="mx-auto flex h-full w-full max-w-[96rem] flex-col gap-4 px-4 py-4 sm:px-6">
        <header className="shrink-0 border-b border-border pb-4">
          ...
          <div className="mt-3 flex flex-wrap gap-2 text-xs">
            ...
            <span className="rounded-full border border-border bg-background px-2.5 py-1 text-muted-foreground">P15 UAT</span>
          </div>
        </header>
              ...
              <div className="grid gap-4 xl:grid-cols-2">
                ...
                <div className="rounded-md border border-primary/30 bg-primary/5 p-4">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <span className="rounded-full border border-primary/30 px-2 py-0.5 text-xs text-primary">P15 自动加载</span>
                  </div>
                  {uatCriteria.length > 0 ? (
                    ...
                    <p className="mt-3 text-sm text-muted-foreground">分析完成后显示后续 P15 的验收任务草稿。</p>
                  )}
                </div>
              </div>
