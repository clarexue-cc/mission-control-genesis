...
import { isFixedDevPreviewMode } from './mc-stable-mode';
...
const FIXED_DEV_AUTH_USER = 'clare-admin'
const FIXED_DEV_AUTH_PASS = 'dev-test-123'

function resolveSeedAuthPasswordFallback(env: NodeJS.ProcessEnv = process.env): string | null {
  return env.AUTH_PASS || (isFixedDevPreviewMode(env) ? FIXED_DEV_AUTH_PASS : null)
}

export function resolveSeedAuthUsername(env: NodeJS.ProcessEnv = process.env): string {
  return env.AUTH_USER || (isFixedDevPreviewMode(env) ? FIXED_DEV_AUTH_USER : 'admin')
}
...
function seedAdminUserFromEnv(dbConn: Database.Database): void {
  ...
  const username = resolveSeedAuthUsername()
