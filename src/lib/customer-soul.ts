...
export async function readP7FilesStatus(tenantId: string): Promise<P7FilesStatus> {
  ...
  const files = await Promise.all(P7_CORE_FILES.map(async (f) => {
    ...
    try {
      ...
      return { name: f.name, displayName: f.displayName, relativePath, exists: true, sizeBytes: info.size }
    } catch {
      return { name: f.name, displayName: f.displayName, relativePath, exists: false, sizeBytes: 0 }
    }
  }))
